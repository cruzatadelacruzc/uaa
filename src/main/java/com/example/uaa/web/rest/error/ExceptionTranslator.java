...
public class ExceptionTranslator implements ProblemHandling {
     ...
     */
    @Override
    public ResponseEntity<Problem> process(@Nullable ResponseEntity<Problem> entity, NativeWebRequest webRequest) {
        if (entity == null) {
            return entity;
        }
        ...
        ProblemBuilder builder = Problem.builder()
                ...
                .with(PATH_KEY, webRequest.getNativeRequest(HttpServletRequest.class).getRequestURI());
    ...
    @Override
    public ResponseEntity<Problem> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, @Nonnull NativeWebRequest webRequest) {
        BindingResult result = ex.getBindingResult();
        List<FieldErrorMV> fieldErrors = result.getFieldErrors().stream()
                .map(f -> new FieldErrorMV(f.getObjectName().replaceFirst("DTO$", ""), f.getField(), f.getCode()))
    ...
    public ResponseEntity<Problem> handleUsernameAlreadyUsedException(UsernameAlreadyUsedException ex, NativeWebRequest request) {
       UsernameAlreadyUsedException problem = new UsernameAlreadyUsedException();
        return create(problem, request, HeaderUtil.createFailureAlert(applicationName,  true,
